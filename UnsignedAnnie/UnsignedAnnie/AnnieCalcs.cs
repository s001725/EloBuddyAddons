    ...
    class AnnieCalcs
    {
        ...
        public static double Q(Obj_AI_Base target)
        {
            return Annie.CalculateDamageOnUnit(target, DamageType.Magical,
                (new float[] { 80, 115, 150, 185, 220 }[Program.Q.Level] + ( 0.8f * Annie.FlatMagicDamageMod)));
        }

        public static float W(Obj_AI_Base target)
        {
            return Annie.CalculateDamageOnUnit(target, DamageType.Magical,
                (new float[] { 70, 115, 160, 205, 250 }[Program.W.Level] + (0.85f * Annie.FlatMagicDamageMod)));
        }
        public static float E(Obj_AI_Base target)
        {
            return Annie.CalculateDamageOnUnit(target, DamageType.Magical,
                (new float[] { 20, 30, 40, 50, 60 }[Program.E.Level] + (0.20f * Annie.FlatMagicDamageMod)));
        }

        public static float R(Obj_AI_Base target)
        {
            return Annie.CalculateDamageOnUnit(target, DamageType.Magical,
                (new float[] { 175, 300, 425 }[Program.R.Level] + (0.80f * Annie.FlatMagicDamageMod)));
        }
